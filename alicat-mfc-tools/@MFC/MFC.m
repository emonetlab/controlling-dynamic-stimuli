classdef MFC < handle
	...
	methods
		...
		function m = MFC()
			...
			corelib.cprintf('text','Connecting to Alicat MFC on port %s\n',m.port)
			...
			try
				...
				corelib.cprintf('red','[FATAL] ')
                corelib.cprintf('text','Could not open the port you specified. Probably the wrong port.\n')
                return
			end
			...
			if ~nargout
                ...
                corelib.cprintf('text','MFC called without assigning to a object. MFC will create an object called "m" in the workspace\n')
                assignin('base','m',m);
            end
		...
		function delete(m)
            if m.verbosity > 5
                corelib.cprintf('green','[INFO] ')
                corelib.cprintf('text','mfc -> delete called \n')
            end
